...
mscMessenger::mscMessenger(){
    ...
    tgtMat2Cmd = new G4UIcmdWithAString("/msc/det/setTargetMaterial2",this);
    tgtMat2Cmd->SetParameterName("tgtMat2", false);
    ...
    tgtLen2Cmd = new G4UIcmdWithADoubleAndUnit("/msc/det/setTargetLength2",this);
    tgtLen2Cmd->SetParameterName("tgtLength2", false);
...
mscMessenger::~mscMessenger(){
  ...
  if(tgtMat2Cmd) delete tgtMat2Cmd;
  if(outNmCmd) delete outNmCmd;
  if(tgtLenCmd) delete tgtLenCmd;
  if(tgtLen2Cmd) delete tgtLen2Cmd;
...
void mscMessenger::SetNewValue(G4UIcommand* cmd, G4String newValue){
  ...
  }else if( cmd == tgtMat2Cmd ){
    fDetCon->SetTargetMaterial2( newValue );
  ...
  }else if( cmd == tgtLen2Cmd ){
    G4double val = tgtLen2Cmd->GetNewDoubleValue(newValue);
    fDetCon -> SetTargetLength2(val);
